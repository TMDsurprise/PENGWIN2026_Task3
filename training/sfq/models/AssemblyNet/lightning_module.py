...
class AssemblyLightningModule(L.LightningModule):
    ...
    def _configure_qsmall_trainable_parameters(self):
        ...
        if len(layers) < 4:
            raise ValueError("Qsmall fine-tuning requires at least four Transformer layers")
    ...
    def on_train_epoch_start(self):
        ...
        if self.qsmall_finetune_enabled and self.trainer.is_global_zero:
            phase = "last2+head" if self.current_epoch < self.qsmall_unfreeze_more_epoch else "last4+head"
    ...
    def loss(self, output_dict: dict, data_dict: dict):
        ...
        if self.output_type == "coords":
            ...
            coord_loss = (
                ...
                + fragment_weight * terms["loss_coord_fragment"]
            )
            ...
            return {"loss": loss, "loss_coord": coord_loss, "rotation_scale": scale, **terms}
    ...
    def configure_optimizers(self):
        ...
        for name, p in self.named_parameters():
            ...
            if self.qsmall_finetune_enabled:
                lr = (
                    self.qsmall_opened_lr
                    if id(p) in self._qsmall_opened_parameter_ids
                    else self.qsmall_base_lr
                )
            else:
