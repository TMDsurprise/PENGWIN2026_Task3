...
def weighted_horn_matrices(input_coords: torch.Tensor, target_coords: torch.Tensor,
                           part_batch_ids: torch.Tensor,
                           point_weights: torch.Tensor | None = None):
    """Differentiable FP32 Horn alignment for exact matched point pairs.

    Fixed power iterations avoid the unstable SVD gradient observed for planar
    pelvic fragments. Gradients intentionally flow only through target_coords.
    """
    source = input_coords.detach().float()
    target = target_coords.float()
    ids = part_batch_ids.detach().long()
    n_parts = int(ids.max().item()) + 1 if ids.numel() else 0
    if n_parts == 0:
        return (
            source.new_zeros((0, 3, 3)), source.new_zeros((0, 3)),
            source.new_zeros((0,)), ids.new_zeros((0,), dtype=torch.bool),
        )

    with torch.autocast(device_type=input_coords.device.type, enabled=False):
        if point_weights is None:
            raw_weight = torch.ones(source.shape[0], device=source.device)
        else:
            raw_weight = point_weights.detach().float().clamp(1e-3, 1.0)
        weight_sum = source.new_zeros(n_parts).index_add_(0, ids, raw_weight)
        weight = raw_weight / weight_sum[ids].clamp_min(1e-8)

        src_mean = source.new_zeros((n_parts, 3)).index_add_(
            0, ids, weight[:, None] * source
        )
        dst_mean = target.new_zeros((n_parts, 3)).index_add_(
            0, ids, weight[:, None] * target
        )
        src_centered = source - src_mean[ids]
        dst_centered = target - dst_mean[ids]
        covariance_points = (
            weight[:, None, None]
            * src_centered[:, :, None]
            * dst_centered[:, None, :]
        )
        covariance = source.new_zeros((n_parts, 9)).index_add_(
            0, ids, covariance_points.reshape(-1, 9)
        ).reshape(n_parts, 3, 3)

        sxx, sxy, sxz = covariance[:, 0, 0], covariance[:, 0, 1], covariance[:, 0, 2]
        syx, syy, syz = covariance[:, 1, 0], covariance[:, 1, 1], covariance[:, 1, 2]
        szx, szy, szz = covariance[:, 2, 0], covariance[:, 2, 1], covariance[:, 2, 2]
        horn = torch.stack(
            (
                torch.stack((sxx + syy + szz, syz - szy, szx - sxz, sxy - syx), dim=-1),
                torch.stack((syz - szy, sxx - syy - szz, sxy + syx, szx + sxz), dim=-1),
                torch.stack((szx - sxz, sxy + syx, -sxx + syy - szz, syz + szy), dim=-1),
                torch.stack((sxy - syx, szx + sxz, syz + szy, -sxx - syy + szz), dim=-1),
            ),
            dim=-2,
        )
        horn = horn / torch.linalg.matrix_norm(
            horn, dim=(-2, -1), keepdim=True
        ).clamp_min(1e-8)
        horn = horn + 1.01 * torch.eye(4, device=source.device)[None]
        quat = horn.new_tensor((1.0, 0.01, 0.02, 0.03)).expand(n_parts, -1).clone()
        quat = quat / torch.linalg.vector_norm(quat, dim=-1, keepdim=True)
        for _ in range(48):
            quat = torch.bmm(horn, quat.unsqueeze(-1)).squeeze(-1)
            quat = quat / torch.linalg.vector_norm(
                quat, dim=-1, keepdim=True
            ).clamp_min(1e-8)

        rotations = quaternion_to_matrix(quat)
        translations = dst_mean - torch.bmm(
            rotations, src_mean.unsqueeze(-1)
        ).squeeze(-1)
        ...
        source_cov = source.new_zeros((n_parts, 9)).index_add_(
            0, ids, source_cov_points.reshape(-1, 9)
        ).reshape(n_parts, 3, 3).detach()
        trace = source_cov.diagonal(dim1=-2, dim2=-1).sum(dim=-1)
        frobenius_sq = source_cov.square().sum(dim=(-2, -1))
        observability = (
        ...
        translations = torch.where(valid[:, None], translations, torch.zeros_like(translations))
        observability = torch.where(valid, observability, torch.zeros_like(observability))
    return rotations, translations, observability, valid
