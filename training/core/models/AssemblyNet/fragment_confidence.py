...
class FragmentConfidenceModule(L.LightningModule):
    ...
    def __init__(
        ...
        finetune_scope: str = "all",
        pa_utility_weight: float = 0.0,
        pa_threshold_normalized: float = 0.05,
        pa_severe_target: bool = False,
        c2_metric_margin: float = 0.05,
        c2_severe_tolerance: float = 0.0,
        c2_severe_threshold: float = 0.2,
    ):
        super().__init__()
        ...
        self.pa_utility_weight = float(pa_utility_weight)
        self.pa_threshold_normalized = float(pa_threshold_normalized)
        self.pa_severe_target = bool(pa_severe_target)
        self.c2_metric_margin = float(c2_metric_margin)
        self.c2_severe_tolerance = float(c2_severe_tolerance)
        self.c2_severe_threshold = float(c2_severe_threshold)
        ...
        self._apply_finetune_scope(finetune_scope)

    def _apply_finetune_scope(self, scope: str):
        scope = str(scope).lower()
        if scope not in {"all", "heads"}:
            raise ValueError(f"finetune_scope must be 'all' or 'heads', got {scope!r}")
        if scope == "heads":
            trainable_prefixes = ("score_head.", "metric_head.", "severe_head.")
            for name, parameter in self.named_parameters():
                if not name.startswith("transformer_model."):
                    parameter.requires_grad_(name.startswith(trainable_prefixes))
        self.finetune_scope = scope
    ...
    def _candidate_targets(
        ...
        normalized_metric_sum = (metrics / scale).sum(dim=-1)
        chamfer_normalized = chamfer / scales[:, None].clamp_min(1e-6)
        pa_risk = torch.sigmoid(
            (chamfer_normalized - self.pa_threshold_normalized) / 0.01
        )
        utility = (
            normalized_metric_sum + self.pa_utility_weight * pa_risk
        ) / (4.0 + self.pa_utility_weight)
        catastrophic = (rot > 30.0) | (trans > 20.0)
        if self.pa_severe_target:
            severe = (catastrophic | (chamfer_normalized >= self.pa_threshold_normalized)).float()
        else:
            severe = catastrophic.float()
    ...
    def _c2_select(self, output, points_per_part):
        """Apply the exact inference C2 gate to validation predictions."""
        scores = output["scores"]
        predicted_metrics = torch.expm1(output["metric_prediction"]).clamp_min(0.0)
        scales = predicted_metrics.new_tensor(self.utility_scales)
        predicted_utility = (predicted_metrics / scales).mean(dim=-1)
        predicted_severe = torch.sigmoid(output["severe_logits"])

        raw = scores.argmax(dim=1)
        rows = torch.arange(raw.shape[0], device=raw.device)
        proposed_utility = predicted_utility[rows, raw]
        current_utility = predicted_utility[:, 0]
        proposed_severe = predicted_severe[rows, raw]
        current_severe = predicted_severe[:, 0]
        improves = proposed_utility + self.c2_metric_margin < current_utility
        severe_safe = (
            (proposed_severe <= current_severe + self.c2_severe_tolerance)
            & (
                (proposed_severe <= self.c2_severe_threshold)
                | (proposed_severe < current_severe)
            )
        )
        accepted = (raw == 0) | (improves & severe_safe)
        selected = torch.where(accepted, raw, torch.zeros_like(raw))

        valid_parts = points_per_part > 0
        part_to_case = torch.nonzero(valid_parts, as_tuple=False)[:, 0].long()
        for case_idx in range(int(points_per_part.shape[0])):
            local = torch.nonzero(part_to_case == case_idx, as_tuple=False).flatten()
            if local.numel() == 0:
                continue
            gated = predicted_utility[local, selected[local]].mean()
            current = predicted_utility[local, 0].mean()
            if not bool(gated < current):
                selected[local] = 0
        return selected
    ...
    def _shared_step(self, data_dict, stage):
        ...
        if stage == "val":
            ...
            c2_selected = self._c2_select(output, data_dict["points_per_part"])
            ...
            c2_selected_utility = self._gather(output["utility"], c2_selected)
            oracle_utility = self._gather(output["utility"], oracle)
            current_utility = output["utility"][:, 0]
            selected_metrics = output["metrics"].gather(
                1, selected[:, None, None].expand(-1, 1, 4)
            ).squeeze(1)
            c2_selected_metrics = output["metrics"].gather(
                1, c2_selected[:, None, None].expand(-1, 1, 4)
            ).squeeze(1)
            ...
            c2_active = c2_selected != 0
            c2_false_accept = c2_active & (c2_selected_utility >= current_utility)
            ...
            validation = {
                ...
                "c2_oracle_regret": (c2_selected_utility - oracle_utility).mean(),
                "c2_top1_accuracy": (c2_selected == oracle).float().mean(),
                "c2_improvement": (current_utility - c2_selected_utility).mean(),
                "c2_accept_rate": c2_active.float().mean(),
                "c2_false_accept_rate": (
                    c2_false_accept.float().sum() / c2_active.float().sum().clamp_min(1.0)
                ),
            }
            ...
            for metric_idx, metric_name in enumerate(metric_names):
                ...
                validation[f"c2_selected_{metric_name}"] = c2_selected_metrics[
                    :, metric_idx
                ].mean()
                for candidate_idx in range(self.num_candidates):
                    validation[
                        f"candidate_{candidate_idx}_{metric_name}"
                    ] = output["metrics"][:, candidate_idx, metric_idx].mean()
            for candidate_idx in range(self.num_candidates):
                validation[f"candidate_{candidate_idx}_utility"] = output[
                    "utility"
                ][:, candidate_idx].mean()
    ...
    def on_validation_epoch_end(self):
        ...
        names = [
            ...
            "val/c2_oracle_regret", "val/c2_top1_accuracy",
            "val/c2_improvement", "val/c2_accept_rate",
            "val/c2_false_accept_rate", "val/c2_selected_tre_mm",
            "val/c2_selected_trans_mm", "val/c2_selected_rot_deg",
            "val/c2_selected_cd_proxy_mm",
        ]
        for candidate_idx in range(self.num_candidates):
            names.append(f"val/candidate_{candidate_idx}_utility")
            for metric_name in ("tre_mm", "trans_mm", "rot_deg", "cd_proxy_mm"):
                names.append(f"val/candidate_{candidate_idx}_{metric_name}")
    ...
    def on_fit_start(self):
        if self.trainer.is_global_zero:
            ...
            print(
                ...
                f"finetune_scope={self.finetune_scope} "
