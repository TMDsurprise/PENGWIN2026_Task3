...
class ClinicalFragmentSamplesCached(FragmentSamples):
    ...
    def __init__(
        ...
        self.case_ids_file = str(case_ids_file or "")
        ...
        if self.split == "train" and self.repeat_factor > 1:
            self.index_map *= self.repeat_factor
        self.reshuffle()
    ...
    def _discover_samples(self) -> list:
        ...
        if self.case_ids_file:
            with open(self.case_ids_file, "r", encoding="utf-8") as handle:
                payload = json.load(handle)
            case_ids = payload.get("case_ids", payload) if isinstance(payload, dict) else payload
            allowed = {str(case_id) for case_id in case_ids}
            dirs = [directory for directory in all_dirs if directory.name in allowed]
            missing = sorted(allowed - {directory.name for directory in dirs})
            if missing:
                raise ValueError(
                    f"{len(missing)} case ids from {self.case_ids_file} are absent under "
                    f"{self.data_root}: {missing[:8]}"
                )
        else:
            n = len(all_dirs)
            split_idx = int(n * 0.9)
    ...
    def _transform_real_pose(self, data, rng, np_rng):
        ...
        # Stored matrices are absolute assembled-to-scattered transforms in the
        # original clinical frame. Applying centering, flipping, or rotation to
        # only the assembled points would invalidate those matrices. The helper
        # below performs the common scattered-frame normalization afterwards.
