...
def sub_sample(
    ...
    Args:
        ...
        allocation_mode: "volume" for the legacy baseline or "area" for Area64
    ...
    for bone_type, fragments in meshdict.items():
        ...
        if allocation_mode not in ("volume", "area"):
            raise ValueError(f"Unknown allocation_mode={allocation_mode!r}")
        measures = []
        for k in frag_keys:
            if allocation_mode == "area":
                value = float(fragments[k]["mesh"].area)
            else:
                value = float(fragments[k]["volume"])
            measures.append(max(value, 1e-3) if np.isfinite(value) else 1e-3)
        ...
        if diff != 0:
            ...
            if diff > 0:
                allocations[max_measure_idx] += diff
            else:
                allocations[max_measure_idx] += diff
                if allocations[max_measure_idx] < min_points:
                    raise ValueError(
                        "Minimum allocation exceeds the per-bone point budget: "
                        f"parts={len(frag_keys)} min_points={min_points} npoints={npoints}"
                    )
