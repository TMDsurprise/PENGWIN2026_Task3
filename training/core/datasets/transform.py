...
QSMALL_HARD_ROTATION_INTERVALS_DEG = (
    (15.0, 45.0),
    (45.0, 90.0),
    (90.0, 150.0),
    (150.0, 180.0),
)
QSMALL_HARD_ROTATION_PROBS = np.array([0.40, 0.35, 0.20, 0.05], dtype=np.float64)
...
def _select_qsmall_target(fragment_diameters_mm, bone_types, np_rng):
    """Select one non-anchor fragment, balancing available SA/LI/RI bones."""
    diameters = np.asarray(fragment_diameters_mm, dtype=np.float64)
    bones = np.asarray(bone_types, dtype=np.int64)
    candidates = np.arange(1, len(diameters), dtype=np.int64)
    if candidates.size == 0:
        return None

    strength = np.clip((120.0 - diameters[candidates]) / 60.0, 0.0, 1.0)
    eligible = candidates[strength > 0.0]
    if eligible.size == 0:
        eligible = np.asarray([candidates[np.argmin(diameters[candidates])]], dtype=np.int64)

    available_bones = np.unique(bones[eligible])
    selected_bone = int(np_rng.choice(available_bones))
    bone_candidates = eligible[bones[eligible] == selected_bone]
    bone_strength = np.clip((120.0 - diameters[bone_candidates]) / 60.0, 0.0, 1.0)
    probabilities = bone_strength + 0.05
    probabilities /= probabilities.sum()
    return int(np_rng.choice(bone_candidates, p=probabilities))


def _sample_qsmall_hard_pose(pointcloud, bone_type, bone_context, np_rng):
    interval_idx = int(np_rng.choice(
        len(QSMALL_HARD_ROTATION_INTERVALS_DEG),
        p=QSMALL_HARD_ROTATION_PROBS,
    ))
    low, high = QSMALL_HARD_ROTATION_INTERVALS_DEG[interval_idx]
    angle_deg = low + (high - low) * np_rng.beta(1.5, 2.0)

    shared_axis = bone_context[int(bone_type)]["axis"]
    local_axis = _random_unit_vector(np_rng)
    axis = 0.65 * shared_axis + 0.35 * local_axis
    axis /= max(np.linalg.norm(axis), 1e-8)
    rot_mat = R.from_rotvec(np.deg2rad(angle_deg) * axis).as_matrix()

    shared_translation = bone_context[int(bone_type)]["translation_dir"]
    local_translation = _random_unit_vector(np_rng)
    translation_dir = 0.60 * shared_translation + 0.40 * local_translation
    translation_dir /= max(np.linalg.norm(translation_dir), 1e-8)
    trans_std = min(22.0, 5.0 + 0.14 * angle_deg)
    trans_cap = min(100.0, 20.0 + 0.55 * angle_deg)
    translation_mag = min(abs(np_rng.normal(0.0, trans_std)), trans_cap)
    translation_rand = translation_dir * translation_mag
...
def process_fragments_clinical_pose(
    ...
    qsmall_target_idx = None
    if sampler_mode == "qsmall_hard":
        qsmall_target_idx = _select_qsmall_target(
            fragment_diameters_mm,
            bone_types,
            np_rng,
        )
    qsmall_target_mask = np.zeros(num_parts, dtype=np.float32)
    if qsmall_target_idx is not None:
        qsmall_target_mask[qsmall_target_idx] = 1.0
    ...
    for part_idx in range(num_parts):
        ...
        if sampler_mode in ("natural", "proposal", "qsmall_hard"):
            if part_idx == 0:
                ...
                if sampler_mode == "qsmall_hard" and part_idx == qsmall_target_idx:
                    _, q_abs, t_abs, rot_abs, angle_deg, translation_mm = _sample_qsmall_hard_pose(
                        pc,
                        bone_types[part_idx],
                        bone_context,
                        np_rng,
                    ...
                    effective_mode = "natural" if sampler_mode == "qsmall_hard" else sampler_mode
