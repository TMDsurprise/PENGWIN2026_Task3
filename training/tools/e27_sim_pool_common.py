...
def rollout_final_pose(
    ...
    *,
    ...
    cumulative = np.repeat(np.eye(4, dtype=np.float64)[None], len(current_points), axis=0)
    amp_enabled = device.type == "cuda"

    for iteration in range(max_iters):
        concatenated = np.concatenate(current_points, axis=0).astype(np.float64)
        centroid = concatenated.mean(axis=0)
        ...
        if quaternion.shape[0]:
            quaternion[0] = quaternion.new_tensor([1.0, 0.0, 0.0, 0.0])
            translation[0].zero_()
        rotation = quaternion_to_matrix(quaternion).detach().cpu().numpy().astype(np.float64)
        translation_np = translation.detach().cpu().numpy().astype(np.float64)
        tinit = build_tinit(centroid, scale)
        tinit_inv = np.linalg.inv(tinit)
        max_translation_mm = 0.0
        for part_index in range(len(current_points)):
            normalized_step = np.eye(4, dtype=np.float64)
            ...
            current_points[part_index] = apply_transform(current_points[part_index], world_step).astype(np.float32)
            current_normals[part_index] = apply_rotation(current_normals[part_index], world_step).astype(np.float32)
