...
class TransformerLayer(nn.Module):
    ...
    def _run_flash_attn_varlen(self, q, k, v, cu_seqlens, max_seqlen, dtype):
        force_standard = os.environ.get("E2_FORCE_STANDARD_ATTENTION", "0") == "1"
        flash_dtype = q.dtype in (torch.float16, torch.bfloat16)
        if flash_attn is None or force_standard or not flash_dtype:
